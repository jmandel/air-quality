...
try {
  ...
  // Get current value (most recent reading)
  ...
  const sixHoursAgo = 1763241429236;
  const oldReading = db.query(
    "SELECT value FROM readings WHERE sensor_id = ? AND ts >= ? ORDER BY ts ASC LIMIT 1"
  ).get(sensorId, sixHoursAgo);
  
  // Calculate trend
  let trend = undefined;
  if (oldReading && oldReading.value) {
    const change = current.value - oldReading.value;
    const percentage = Math.abs((change / oldReading.value) * 100);
  ...
  // Get recent data for chart (last 6 hours)
