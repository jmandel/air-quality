...
const FALLBACK_DEVICE_URL = "http://10.0.0.37/";
const MAX_LOG_ENTRIES = 200;

const deviceInput = document.getElementById("device-url") as HTMLInputElement;
const startButton = document.getElementById("start-button") as HTMLButtonElement;
const stopButton = document.getElementById("stop-button") as HTMLButtonElement;
const statusBadge = document.getElementById("upload-status") as HTMLElement;

const statReceived = document.getElementById("stat-received") as HTMLElement;
const statUploaded = document.getElementById("stat-uploaded") as HTMLElement;
const statDuplicates = document.getElementById("stat-duplicates") as HTMLElement;
const statLastPost = document.getElementById("stat-last-post") as HTMLElement;
const statBuffer = document.getElementById("stat-buffer") as HTMLElement;
...
let lastPostTs: number | null = null;
let pendingBytes = 0;

function normalizeDeviceUrl(raw: string): string {
  return raw.replace(/\/+$/, "");
}

function estimateReadingSize(reading: Omit<PendingReading, "bytes">): number {
  // Rough estimate using JSON length to capture string payload impact
  return JSON.stringify(reading).length;
}

function setStatus(state: "idle" | "connecting" | "streaming" | "error") {
...
function appendLog(message: string) {
  ...
  while (logArea.childElementCount > MAX_LOG_ENTRIES) {
    logArea.removeChild(logArea.firstChild as Node);
  }
...
function handleStateEvent(event: MessageEvent) {
  if (!shouldRun) return;
  let payload: any;
  try {
  ...
  const eventId =
    typeof event.lastEventId === "string" && event.lastEventId.trim().length > 0
      ? event.lastEventId
      : `${now}:${payload.id}`;
...
function connectToDevice() {
  if (!shouldRun || !currentDeviceUrl) return;
  ...
  eventSource.addEventListener("state", (event) =>
    handleStateEvent(event as MessageEvent),
  );
...
async function flushPending() {
  ...
  try {
    ...
    const inserted =
      typeof body?.inserted === "number" ? body.inserted : batch.length;
    const duplicates =
...
function startUpload() {
  ...
  if (flushTimer === null) {
    flushTimer = window.setInterval(flushPending, 1_000);
  }
}
